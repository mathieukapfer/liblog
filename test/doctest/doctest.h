...
#pragma warning(pop)
    ...
    DOCTEST_INTERFACE void logTestSubcase();
...
#if 0 /* HACK by MATK: display subcase title */
...
#include <sys/sysctl.h>
    ...
    void logTestStart(const TestCase& tc) {
        ...
        DOCTEST_SNPRINTF(loc, DOCTEST_COUNTOF(loc), "\n%s:%d:", fileForOutput(tc.m_file),
        ...
        DOCTEST_SNPRINTF(ts2, DOCTEST_COUNTOF(ts2), "'%s' ", tc.m_test_suite);
        ...
        DOCTEST_SNPRINTF(n2, DOCTEST_COUNTOF(n2), "'%s' ", tc.m_name);
        ...
        DOCTEST_SNPRINTF(d2, DOCTEST_COUNTOF(d2), "%s", tc.m_description);
...
void logTestSubcase() {
        ...
        if (subcasesStack.size()) {
          // print location of last subcase macro
          char loc[DOCTEST_SNPRINTF_BUFFER_LENGTH];
          const char depth = subcasesStack.size()-1;
          DOCTEST_SNPRINTF(loc, DOCTEST_COUNTOF(loc), "\n%s:%d:",
                           subcasesStack[depth].m_signature.m_file,
                           subcasesStack[depth].m_signature.m_line);
          DOCTEST_PRINTF_COLORED(loc, Color::LightGrey);
          char subcase[DOCTEST_SNPRINTF_BUFFER_LENGTH];
          DOCTEST_SNPRINTF(subcase, DOCTEST_COUNTOF(subcase), "SUBCASE[%d]:", depth);
          DOCTEST_PRINTF_COLORED(subcase, Color::Yellow);
        }
        // print the subcases stack
        for(unsigned i = 0; i < subcasesStack.size(); ++i) {
            if(subcasesStack[i].m_signature.m_name[0] != '\0') {
                char subcase[DOCTEST_SNPRINTF_BUFFER_LENGTH];
                DOCTEST_SNPRINTF(subcase, DOCTEST_SNPRINTF_BUFFER_LENGTH , "[%d]'%s' ",
                                 i,
        ...
        printToDebugConsole(subcaseStuff + "\n");
    }
    ...
    void printHelp() {
        ...
        std::printf(" -t,   --title=<bool>                  print test suite, case and subcase title in output\n");
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                                        \
    ...
    DOCTEST_PARSE_AS_BOOL_OR_FLAG(dt-title, dt-t, title, true);
